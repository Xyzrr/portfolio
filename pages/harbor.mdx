...
I started by trying to implement my own video chat infrastructure off of the open source library [Mediasoup](https://mediasoup.org/). It quickly became evident that this was going to be more effort than it was worth to start out, so I tried the SaaS solution from Twilio. It turned out to be terrible; even on 200Mb/s wifi, the latency was often >1 second and the screen sharing framerate was <5fps. And of course, customer support was bureaucratic. So I turned to the fledgling startup [Daily](https://www.daily.co/), which turned out to be better than Twilio in almost all dimensions. But it wasn't without problems.

Daily allowed clients to specify which call participants they want to receive streams from, but not which participants they want to _send_ streams to. My use case would have greatly benefitted from the latter because a client should only send streams to nearby participants. I was forced to instead make clients opt out of receiving streams from faraway participants, which presents a security problem because any user could hack their client to listen in on distant participants without them knowing.

Unfortunately I was never able to solve this problem; the only way would have been to roll my own infrastructure.
